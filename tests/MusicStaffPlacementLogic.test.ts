import { MusicStaffPlacementLogic, MusicStaffPlacementLogicConfig } from '../src/lib/core/MusicStaffPlacementLogic'
...
const makeConfig = (beatsPerMeasure: number, beatDuration: NotationType): MusicStaffPlacementLogicConfig => ({
    accidentalSize: 12,
    noteSize: 14,
    noteSpacing: 20,
    spaceHeight: 12,
...
describe('MusicStaffPlacementLogic.getTies', () => {
    it('creates a tie between consecutive same-pitch notes when tieToNext is true', () => {
        const config = makeConfig(4, NotationType.Quarter)
        const notations = [
            new Note(NotationType.Quarter, Pitch.C4, 0, 0, true),
            new Note(NotationType.Quarter, Pitch.C4, 1 / 4)
        ]

        const ties = MusicStaffPlacementLogic.getTies(notations, config)
...
describe('MusicStaffPlacementLogic.getHorizontalBeams', () => {
    it('beams four eighth notes together in 4/4', () => {
        ...
        const beams = MusicStaffPlacementLogic.getHorizontalBeams(notations, config)
    ...
    it('breaks beaming at dotted notes', () => {
        ...
        const beams = MusicStaffPlacementLogic.getHorizontalBeams([dotted, next], config)
    ...
    it('groups eighth notes by three in 6/8 compound time', () => {
        const config = makeConfig(6, NotationType.Eighth)
        const notations = [
            new Note(NotationType.Eighth, Pitch.C4, 0),
            new Note(NotationType.Eighth, Pitch.D4, 1 / 8),
            new Note(NotationType.Eighth, Pitch.E4, 2 / 8),
    ...
    it('groups eighth notes as 3+2 in 5/8', () => {
        const config = makeConfig(5, NotationType.Eighth)
        const notations = [
            new Note(NotationType.Eighth, Pitch.C4, 0),
            new Note(NotationType.Eighth, Pitch.D4, 1 / 8),
            new Note(NotationType.Eighth, Pitch.E4, 2 / 8),
